...
public class RolDAOImpl implements RolDAO {
    ...
    public void saveRol(Rol rol) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            session.save(rol); // Hibernate manejará automáticamente la relación Rol-Permiso.
            transaction.commit();
        } catch (Exception e) {
    ...
    public void updateRol(Rol rol) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            transaction = session.beginTransaction();
            session.update(rol); // Asegúrate de que rol incluya permisos en el set.
            transaction.commit();
        } catch (Exception e) {
            if (transaction != null && transaction.isActive()) {
                transaction.rollback();
    ...
    public List<Rol> getActivosRoles() {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            e.printStackTrace();
            throw new RuntimeException("Error obteniendo roles activos", e);
        }
    }
